"""Visual sanity check for noise.py — compose 3x4 grid of (sample, severity)."""
...
def make_grid(out_path: Path, seed: int = 42, target_h: int = 320) -> None:
    ...
    for label, path in SAMPLES:
        ...
        # downscale to target_h for grid composability
        h, w = img.shape[:2]
        scale = target_h / h
        ...
        for s in (1, 2, 3):
            perturbed = noise.apply(img_small, severity=s, seed=seed)
            cells.append(annotate(perturbed, f"{label}: noise s{s}"))
...
def main() -> None:
    ...
    ap.add_argument("--out", type=Path, default=Path("docs/noise_examples.png"))
